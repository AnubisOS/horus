...
from os import getppid, readlink
...
class Horus:
    ...
    def _get_shell(self) -> None:
        ...
        """
        self.shell = readlink(f"/proc/{getppid()}/exe").rsplit("/")[-1]
        self.shell.capitalize()

    def _get_OS(self) -> None:
        """
        Determine some OS data like, host name ,
        kernel name , kernel version and OS name.
        ...
        data = (
            "{"
            + getoutput(
                "hostnamectl --json=pretty | grep -e 'KernelName' -e \
                'KernelRelease' -e 'Hostname' -e \
                'OperatingSystemPrettyName' -w"
            )
            .replace("\n", "")
            .replace("\t", "")[:-1]
            + "}"
